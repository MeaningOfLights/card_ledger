...
$output = [string] (& dotnet test $repoRoot/CardLedger.sln --collect:"XPlat Code Coverage" 2>&1)
...
reportgenerator -reports:"$repoRoot/**/coverage.cobertura.xml" -targetdir:"$dir/coveragereport" -reporttypes:Html -historydir:$dir/CoverageHistory -assemblyfilters:"-*.Tests"
